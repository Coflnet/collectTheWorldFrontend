...
class LeaderboardApi {
  ...
  /// Performs an HTTP 'GET /api/leaderboard/boards' operation and returns the [Response].
  Future<Response> getAvailableLeaderboardsWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/api/leaderboard/boards';
  ...
  Future<BoardNames?> getAvailableLeaderboards() async {
    final response = await getAvailableLeaderboardsWithHttpInfo();
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'BoardNames',) as BoardNames;
  ...
  /// Performs an HTTP 'GET /api/leaderboard/me/ranks' operation and returns the [Response].
  Future<Response> getRanksWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/api/leaderboard/me/ranks';
  ...
  Future<RankSummary?> getRanks() async {
    final response = await getRanksWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'RankSummary',) as RankSummary;
