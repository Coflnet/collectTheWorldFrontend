...
class AuthApi {
  ...
  /// * [AnonymousLoginRequest] anonymousLoginRequest:
  ///   
  Future<Response> loginDeviceWithHttpInfo({ AnonymousLoginRequest? anonymousLoginRequest, }) async {
    // ignore: prefer_const_declarations
    final path = r'/api/auth/deviceId';
  ...
  /// * [AnonymousLoginRequest] anonymousLoginRequest:
  ///   
  Future<TokenResponse?> loginDevice({ AnonymousLoginRequest? anonymousLoginRequest, }) async {
    final response = await loginDeviceWithHttpInfo( anonymousLoginRequest: anonymousLoginRequest, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
